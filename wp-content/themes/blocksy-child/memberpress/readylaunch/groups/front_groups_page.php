    ...
    <div class="mepr-price-boxes mepr-<?php echo count($products); ?>-col">
        ...
        if (!empty($products)) {
            foreach ($products as $product) {
                ...
                ?>
                <div id="mepr-price-box-<?php echo $product->ID; ?>"
                    class="mepr-price-box <?php echo $group_classes_str; ?>">
                    <?php if ($product->is_highlighted) :
                        // _e('Most Popular', 'memberpress');
                        ?>
                        <div class="mepr-most-popular">Popular</div>
                    <?php endif ?>
                    ...
                    <div class="mepr-price-box-content">
                        <div class="mepr-price-box-head">
                            ...
                            <?php if ($preview) : ?>
                                ...
                                <span class="mepr-price-box-price-loading"><img
                                        src="<?php echo admin_url('/images/wpspin_light.gif'); ?>"/></span>
                            <?php elseif ($product->pricing_display !== 'none') : ?>
                                <div class="mepr-price-box-price">
                                    ...
                                    if ($product->pricing_display == 'auto') {
                                        $mepr_options = MeprOptions::fetch();
                                        global $wp_locale;
                                        $price = $product->adjusted_price($mepr_coupon_code);
                                        $regex_dp = preg_quote($wp_locale->number_format['decimal_point'], '#');
                                        $fprice = MeprAppHelper::format_currency($price, false);
                                        $fprice = preg_replace("#([{$regex_dp}]000?)([^0-9]*)$#", '$2', (string)$fprice);
                                        /** $product->period_type | months, weeks */
                                        $subPrice =  $product->period_type === 'months' ? 'mo' : $product->period_type;

                                        echo "<h3>" . $mepr_options->currency_symbol . $fprice.  "</h3>";
                                        echo "<p>per product / ".$subPrice."</p>";
                                    } else {
                            ...
                            <?php if ($product->post_content) { ?>
                                <div class="mepr-post-content"><?= $product->post_content; ?></div>
                            <?php } ?>
                            ...
                            <div class="mepr-price-box-button">
                                ...
                                if (
                                    ...
                                    ?>
                                    <a <?php echo 'href="' . $product->access_url . '"'; ?>
                                        class="<?php echo MeprGroupsHelper::price_box_button_classes($group, $product, true); ?>"><?php _e('View', 'memberpress'); ?></a>
                                <?php else : ?>
                                    <a <?php echo $active ? 'href="' . $product->url() . '"' : ''; ?>
                                        class="<?php echo MeprGroupsHelper::price_box_button_classes($group, $product, $active); ?>"
